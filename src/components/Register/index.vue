    ...
    >
      ...
      <template #footer>
        <div class="dialog-footer">
          <el-button @click="handleCaptchaClose" :disabled="isRegistering">取消注册</el-button>
        </div>
      </template>
    </el-dialog>
  </div>
...
async function handleSlideComplete({ captchaKey, slideX }: { captchaKey: string; slideX: number }) {
  try {
    ...
    const encryptedPassword = rsaEncrypt(registerForm.password, publicKey.value)
    const encryptedConfirmPassword = rsaEncrypt(registerForm.confirmPassword, publicKey.value)
...
function handleCaptchaClose() { if (!isRegistering.value) showCaptcha.value = false }
